...
- [x] **C1 - Selection policies.** Write failing tests for priority, weighted, and low-latency prior selection, then implement pure routing. Files: `src/lucy/router.py`, `tests/test_router.py`. Verify: `docker compose run --rm lucy-api pytest tests/test_router.py -q -k selection` -> selected tests pass.
- [x] **C2 - Streaming failover.** Add tests for failover before first token and no failover after first token, then implement `RoutingLlmProvider`. Files: `src/lucy/router.py`, `tests/test_router.py`. Verify: `docker compose run --rm lucy-api pytest tests/test_router.py -q` -> all router tests pass.
- [x] **C3 - Observability and gates.** Add span assertions for fallback/cooldown metadata, update docs if needed, and run full gates. Files: `src/lucy/router.py`, `tests/test_router.py`, `docs/model-registry-maintenance.md`. Verify: `docker compose run --rm lucy-api pytest` -> full suite green.
...
- [x] `docker compose run --rm lucy-api pytest tests/test_router.py -q` -> router tests pass
...
- Pre-token events are buffered until the first `TokenDelta`, preventing partial
  tool or usage events from leaking from a route that fails over.
- Provider exception text is excluded from routing spans and aggregate errors;
  telemetry records only the stable exception type so credentials embedded in
  upstream messages cannot leak.
- `RouterSettings` validates non-negative cooldown and EWMA alpha in `(0, 1]`.
- STT/TTS use the same pure `Router.select()` at session start. Mid-call media
  failover remains gateway-owned per the card failure protocol and card 51.
- Final evidence: 12 router tests and 455 root tests passed; Docker ruff, format,
  and mypy gates were clean.

## Review evidence

- code-reviewer: PASS - failover boundary, cooldown state, and typed errors reviewed; no P0-P3 findings.
- test-auditor: PASS - all policies, pre/post-token failures, EWMA, cooldown, and sensitive-error branches covered.
- docs-reviewer: PASS - registry maintenance guide now states route-key, prior, EWMA, and media-failover assumptions.
- simplicity-reviewer: PASS - one generic router plus one LLM wrapper; no provider-specific branches or ABI changes.
- security-reviewer: PASS - route spans omit raw exception messages and tests prove sensitive text is not emitted.
