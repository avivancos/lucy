...
REPLAY_CREDENTIAL_FIELDS = frozenset(
    {"authorization", "api_key", "xi-api-key", "xi_api_key", "token"}
)
...
class ReplayTransport:
    def __init__(self, frames: List[RecordedFrame]) -> None:
    ...
    async def send(self, payload: dict) -> None:
        ...
        expected = _comparable_payload(frame.payload)
        actual = _comparable_payload(payload)
        if actual != expected:
    ...
    def _mismatch(self, expected: object, actual: object) -> FixtureMismatch:
        return FixtureMismatch(
            ...
            % (self._path, self._index, expected, actual)
        )
    ...
    def visit(value: object) -> object:
        if isinstance(value, dict):
            return {
                key: visit(item)
                for key, item in value.items()
                if key.casefold() not in REPLAY_CREDENTIAL_FIELDS
            }
        if isinstance(value, list):
            return [visit(item) for item in value]
        return value

    stripped = visit(payload)
